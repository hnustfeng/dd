        ...
        (
            data && data.length > 0 ?
                <div className=' box-border py-[24px] px-[32px] border border-solid border-[#EAEBF0] rounded-[5px]  '>
                    ...
                    {title == "Your supplies" || title == "Assets to supply" || title == "Assets to borrow"? (columns[0].data.map(((item, index) => (
                        ...
                        <div className='block md:hidden '>
                            ...
                            <div className='flex justify-between mb-[16px]'>
                                ...
                                <div>
                                    <div className='text-[12px] text-[#5F6D7E] mb-[5px]'>{columns[4].columnsName}</div>
                                    <div className='text-[16px] font-bold'>{columns[4].data[index]}</div>
                                </div>
                            </div>

                            <div className='flex flex-row-reverse text-[14px] font-bold '>

                                <Button className='px-[9.5px] py-[5.5px] rounded-[6px] border border-solid border-[#b0b6bd] ' disabled={!columns[5].button2IsAble[index]} onClick={() => columns[5].button2Func(index)}>{columns[5].button2}</Button>
                                <button className={columns[5].button1IsAble[index] ? 'mr-[6px] px-[9.5px] py-[5.5px] bg-[#F4B512] text-[white] rounded-[6px] border-none ' : 'mr-[6px] px-[9.5px] py-[5.5px] bg-[#F4B512]/[0.6] text-[white] rounded-[6px] border-none'} disabled={!columns[5].button1IsAble[index]} onClick={() => columns[5].button1Func(index)}>{columns[5].button1}</button>
                                {/* <Button className=' text-[1.2rem] bg-[#F4B512] text-[white] rounded-[0.5rem] py-[0.3rem] px-[0.6rem] mr-[0.4rem] cursor-pointer border-none' onClick={() => { setOperation("Repay"); setBoxData(record); setOpen(true) }}>Repay</Button> */}
                                {/* <Button className=' text-[1.2rem] py-[0.3rem] px-[0.5rem] rounded-[0.6rem] border border-solid border-[#b0b6bd] cursor-pointer flex items-center justify-center' disabled={!record.borrowIsable} onClick={() => { setOperation("Borrow"); setBoxData(record); setOpen(true) }}>Borrow</Button> */}
                            </div>
                        </div>
