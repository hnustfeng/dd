...
import { Modal, Input, Button, Steps, ConfigProvider, message } from "antd"
...
import { EthereumCode } from '../utils/constants';
...
export default function Trasaction({ title, open, setOpen, data, web3modal, address, chain }) {
    ...
    var debt 
    if(title == "Repay"){
        debt = data.totalBorrow.toString().length>data.decimals?data.totalBorrow.toString().substring(0,data.totalBorrow.toString().length-data.decimals)+"."+data.totalBorrow.toString().substring(data.totalBorrow.toString().length-data.decimals):data.totalBorrow.toString().padStart(data.decimals+1,'0').substring(0,1)+"."+data.totalBorrow.toString().padStart(data.decimals+1,'0').substring(1)
    }
            ...
            if (result.code != "ACTION_REJECTED") {
                try {
                    if(result.error.data.message){
                        message.error(result.error.data.message + " Please try again!")
                    }else{
    ...
    const handleBorrow = async () => {
        ...
        if (result == "") {
            ...
            await post("/v1/updateBorrow", { "asset": data.ERC20Name, "balance": reserveData.principalBorrowBalance.toString(), "user": address, "rateMode": rateMode, "net": chain.id == EthereumCode ? "Ethereum" : "Scroll" ,"originfee":reserveData.originationFee.toString()})
    ...
    const handleRepay = async () => {
        ...
        if(debt ==  value){
            addValue = data.totalBorrow
        }else{
            if(data.assetAddress==ETHEREUM_ADDRESS){
                var Value = (Number(value)*10000).toFixed(0)
                addValue=BigNumber.from(Value).mul(BigNumber.from(10).pow(14))
              }else{
                var Value = (Number(value)*100).toFixed(0)
                addValue=BigNumber.from(Value).mul(BigNumber.from(10).pow(data.decimals -2))
              }
        }
        ...
        if (result == "") {
            ...
            await post("/v1/updateBorrow", { "asset": data.ERC20Name, "balance": userData.principalBorrowBalance.toString(), "user": address, "net": chain.id == EthereumCode ? "Ethereum" : "Scroll","originfee":userData.originationFee.toString() })
            window.location.reload()
        } else {
            setApproveStatu("wait")
            setDoneStatu("wait")
    ...
    const checkValueSupply = (event) => {
        if(data.name[0]=="ETH"){
            event.target.value = event.target.value.match(/^\d*(\.?\d{0,4})/g)
        }else{
            ...
            <div><Modal title={title + " " + data.name[0]} open={open} onOk={handleOk} onCancel={handleCancel} footer="">
                ...
                <div className='border border-solid border-[#E5E3E6] rounded-[6px] box-border py-[12px] px-[16px] mb-[20px]'>
                    <div className='flex justify-between mt-[12px]'>
                        ...
                        {Number(data.healthFactor.charAt(0)) >= 1 || data.healthFactor == "\u00A0\u00A0\u00A0\u00A0\u221E" ? <div className='font-bold text-[#00B600]'>{data.healthFactor}</div> : <div className='font-bold text-[#FF0000]'>{data.healthFactor}</div>}
                    </div>
        ...
        return (
            <div><Modal title={title} open={open} onOk={handleOk} onCancel={handleCancel} footer="">
                ...
                <div className='relative zdy-input border border-solid border-[#E5E3E6] rounded-[6px]'>
                    ...
                    <div className='flex justify-between box-border px-[10px] mb-[5px]'><div className='text-[#D7D7D7] '>${input} </div> <div><span className='text-[14px] text-[#919AA6] whitespace-nowrap '>Wallet Balance</span> {data.WalletBalance} <span className='text-[#F4B512] cursor-pointer' onClick={() => { clickMax(parseFloat(data.WalletBalance) < parseFloat(data.balance) ? data.WalletBalance : debt) }}>Max</span></div></div>
                </div>
                <div className='mt-[20px] mb-[12px] text-[16px] font-semibold'>Transaction OverView</div>
                <div className='border border-solid border-[#E5E3E6] rounded-[6px] box-border py-[12px] px-[16px] mb-[20px]'>
                    <div className='flex justify-between mb-[12px]'>
                        <div className='text-[#919AA6]'>Estimate Debt</div>
                        <div className='font-bold'>{debt} {data.name[1]}</div>
                    </div>
                    <div className='flex justify-between mb-[12px]'>
