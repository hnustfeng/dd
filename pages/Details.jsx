...
import { useAccount, useConnect, useSwitchChain, useSwitchNetwork, } from 'wagmi'
...
export default function EthDetails() {
    ...
    const { address, isConnecting, isDisconnected, chain, chainId, connector } = useAccount()
    ...
    const fetchData = async () => {
        try {
            ...
            assetDate.supplyapy = (BigNumber.from(data['SupplyAPY']).div(BigNumber.from(10).pow(23)).toNumber() / 100).toFixed(2)
            assetDate.apys = (BigNumber.from(data['BorrowAPYs']).div(BigNumber.from(10).pow(23)).toNumber() / 100).toFixed(2)
            assetDate.apyv = (BigNumber.from(data['BorrowAPYv']).div(BigNumber.from(10).pow(23)).toNumber() / 100).toFixed(2)
            ...
            if (netId == EthereumCode && !BigNumber.from(data["TotalBorrowed"]).add(BigNumber.from(data['TotalSupplied'])).isZero()) {
                miningDPR = (29166700 / (BigNumber.from(data["TotalBorrowed"]).add(BigNumber.from(data['TotalSupplied']))).toNumber() * 3 / 7).toFixed(2)
            }
            if (netId == ScrollCode && !BigNumber.from(data["TotalBorrowed"]).add(BigNumber.from(data['TotalSupplied'])).isZero()) {
                if (data["TokenAddress"] == "0x5300000000000000000000000000000000000004") {
                    miningDPR = (29166700 * 0.3 / (BigNumber.from(data["TotalBorrowed"]).add(BigNumber.from(data['TotalSupplied']))).toNumber()).toFixed(2)
                } else if (data["TokenAddress"] == "0xf610A9dfB7C89644979b4A0f27063E9e7d7Cda32") {
                    miningDPR = (29166700 * 0.7 / (BigNumber.from(data["TotalBorrowed"]).add(BigNumber.from(data['TotalSupplied']))).toNumber()).toFixed(2)
                } else {
                    miningDPR = (29166700 / (BigNumber.from(data["TotalBorrowed"]).add(BigNumber.from(data['TotalSupplied']))).toNumber()).toFixed(2)
                }
            }
            assetDate.miningDPR = miningDPR
    ...
    const fetchAssetData = async () => {
        try {
            ...
            if (BigNumber.from(data['PriceOracle']).eq(BigNumber.from(0))) {
                if (asset == "ETH") {
                    assetDate.availableBorrow = (0).toFixed(4)
                } else {
                    assetDate.availableBorrow = (0).toFixed(2)
                }
            } else {
                if (asset == "ETH") {
                    let bigAvailableBorrow = BigNumber.from(availableBorrow).mul(BigNumber.from(10000)).div(BigNumber.from(data['PriceOracle']))
                    let bigAvailableLiquidity = BigNumber.from(data["AvailableLiquidity"]).div(BigNumber.from(10).pow(data["Decimals"] - 4))
                    if (bigAvailableBorrow.lt(bigAvailableLiquidity)) {
                        assetDate.availableBorrow = (bigAvailableBorrow.toNumber() / 10000).toFixed(4)
                    } else {
                        assetDate.availableBorrow = ((bigAvailableLiquidity.toNumber()) / 10000).toFixed(4)
                    }
                } else {
            ...
            const borrowdata = {}
            borrowdata.name = [data["Name"], data["Asset"]]
            ...
            var supplydata = {}
            supplydata.name = [data["Name"], data["Asset"]]
    ...
    //         setConnectReady(-1)
    //         }else{
    //             setConnectReady(connectReady+1)
    //         }
    //     }else{
    //         setConnectReady(connectReady+1)
    //     }
    ...
    useEffect(() => {
        if (asset !== "" && network !== "" && address && connector?.getProvider && connectReady == -1) {
            fetchAssetData();
        }
    }, [chain, asset, address, connector, connectReady])
            ...
            <div className=' box-border py-[64px] px-[11.2rem] mb-[64px]'>
                ...
                <div className='flex justify-between flex-wrap md:w-[80rem] mb-[64px] '>
                    <div className='w-[48%] md:flex-1 mt-[16px] md:mt-[0px]'>
                        ...
                        <div className='text-[#272D37] text-[22px] font-bold '>{detailData.coin}</div>
                    </div>
                    ...
                    <div className='basis-[64%] box-border py-[24px] px-[32px] border-solid border-[1px] border-[#EAEBF0]'>
                        ...
                        <div className='border-[1px] border-solid border-[#EAEBF0] box-border p-[20px]'>
                            <div className='md:w-[60%]  flex justify-between mb-[3rem] items-center'>
                                ...
                                <div >
                                    <div className='mb-[1rem] text-[#5F6D7E] md:pl-[30px] text-[14px]'>Mining DPY</div>
                                    <div className=' font-bold text-[16px] h-[40px] flex justify-center items-center md:pl-[30px] border border-solid border-[white] border-l-[#EAEBF0]'>{detailData.miningDPR}%</div>
                                </div>
                            </div>
                        ...
                        <div className='border-[1px] border-solid border-[#EAEBF0] box-border p-[20px]'>
                            <div className='md:w-[60%]  flex justify-between mb-[30px]'>
                                ...
                                <div className=' md:justify-center md:items-center px-[15px] md:pl-[30px] border border-solid border-[white] border-l-[#EAEBF0]'>
                                    <div className='mb-[1rem] text-[#5F6D7E] text-[14px]'>Mining DPY</div>
                                    <div className='font-bold text-[16px]'>{detailData.miningDPR}%</div>
                                </div>
                            </div>
                        ...
                        : (<div className='basis-[32%] box-border py-[24px] px-[32px] border-solid border-[1px] border-[#EAEBF0]'>
                            ...
                            <div className='box-border p-[10px] border-solid border-[0.5px] border-[#EAEBF0]'>
                                ...
                                <div className='flex justify-between items-center'>
                                    ...
                                    <div><button className='box-border bg-[#F4B512] p-[10px]  rounded-[6px] text-[white] font-semibold cursor-pointer text-[15px] border-none' onClick={() => { setTargetChain(netId); setOperation("Supply"); setBoxData(supplyData); setOpen(true) }}>Supply</button></div>
                                </div>
                                <div className='flex justify-between mt-[20px] items-center'>
                                    ...
                                    <div><button className='box-border bg-[#F4B512] p-[10px]  rounded-[6px] text-[white] font-semibold cursor-pointer text-[15px] border-none' onClick={() => { setTargetChain(netId); setOperation("Borrow"); setBoxData(borrowData); setOpen(true); console.log(boxData); }}>Borrow</button></div>
                                </div>
                            </div>
                        </div>)}
                </div>
