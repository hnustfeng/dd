...
export default function EthMarket() {
    ...
    const fetchData = async () => {
        try {
            ...
            for (let index = 0; index < marketdata.length; index++) {
                ...
                if(!BigNumber.from(marketdata[index]["TotalBorrowed"]).add(BigNumber.from(marketdata[index]['TotalSupplied'])).isZero()){
                    if(marketdata[index]["TokenAddress"] == "0x5300000000000000000000000000000000000004"){
                        miningDPR = (29166700*0.3/(BigNumber.from(marketdata[index]["TotalBorrowed"]).add(BigNumber.from(marketdata[index]['TotalSupplied']))).toNumber()).toFixed(2)
                    }else if(marketdata[index]["TokenAddress"] == "0xf610A9dfB7C89644979b4A0f27063E9e7d7Cda32"){
                        miningDPR = (29166700*0.7/(BigNumber.from(marketdata[index]["TotalBorrowed"]).add(BigNumber.from(marketdata[index]['TotalSupplied']))).toNumber()).toFixed(2)
                    }else{
                        miningDPR = (29166700/(BigNumber.from(marketdata[index]["TotalBorrowed"]).add(BigNumber.from(marketdata[index]['TotalSupplied']))).toNumber()).toFixed(2)
                    }
                }
                    ...
                    assetRow.push(
                        {
                            ...
                            supply_apy: (BigNumber.from(marketdata[index]['SupplyAPY']).div(BigNumber.from(10).pow(23)).toNumber() / 100),
                            ...
                            variable: (BigNumber.from(marketdata[index]['BorrowAPYv']).div(BigNumber.from(10).pow(23)).toNumber() / 100),
                            stable: (BigNumber.from(marketdata[index]['BorrowAPYs']).div(BigNumber.from(10).pow(23)).toNumber() / 100),
                            miningDPR:miningDPR,
                        }
                    )
                } else {
    ...
    const columns = [
        {
            title: <div className='text-[#c8cad3] text-[16px] relative bottom-[-10px] '>Asset</div>,
            dataIndex: 'asset',
            key: 'asset',
        ...
        {
            title: <div className=' text-[#c8cad3] text-[16px] relative bottom-[-10px]  text-center' text-center>Total supplied</div>,
        ...
        // {
        //     title: <div className=' text-[#c8cad3] text-[16px] relative bottom-[-10px] text-center'>Supply APY</div>,
        //     dataIndex: 'supply_apy',
        //     key: 'supply_apy',
        //     render: (text) => (<div className='font-bold text-[16px]'><div className='text-center'>{text}%</div></div>)
        // },
        {
            title: <div className=' text-[#c8cad3] text-[16px] relative bottom-[-10px] text-center'>Total borrowed</div>,
            dataIndex: 'total_borrowed',
            key: 'total_borrowed',
            render: (text) => (<div className='font-bold text-[16px]'><div className='text-center'>{text[1]}</div><div className='font-normal text-[#c8cad3] text-[16px] text-center'>${text[0]}</div></div>)
        },
        {
            title: <div className=' text-[#c8cad3] text-[16px] relative bottom-[-10px] text-center'>Mining DPR</div>,
            dataIndex: 'miningDPR',
            key: 'miningDPR',
            render: (text) => (<div className='font-bold text-[16px]'><div className='text-center'>{text}%</div></div>)
        },
        {
            title: <div className=' text-[#c8cad3] text-[16px] relative bottom-[-10px] text-center'>Borrow APY,variable</div>,
            dataIndex: 'variable',
            key: 'variable',
            render: (text) => (<div className='font-bold text-[16px] text-center'>{text}%</div>)
        },
        {
            title: <div className=' text-[#c8cad3] text-[16px] relative bottom-[-10px] text-center'>Borrow APY,stable</div>,
            dataIndex: 'stable',
            key: 'stable',
            render: (text) => (<div className='font-bold text-[16px] text-center'>{text}%</div>)
        },
        {
            ...
            render: (text, record) => (<div className='flex font-semibold '>
                ...
                <Button className='text-[14px] py-[5px] px-[10px] rounded-[6px] border border-solid border-[#EAEBF0] cursor-pointer flex items-center justify-center' onClick={() => router.push('/Details?asset=' + record.asset[0] + '&&net=Scroll')}>Details</Button>
            </div>)
        }
    ];
    return (
        <div className='min-h-full'>
            ...
            <div className='box-border  py-[64px] px-[32px] md:px-[11.2rem] rounded-[1rem]'>
                ...
                <div className='hidden md:flex mt-[16px] mb-[8px] text-[16px] font-normal text-[#5F6D7E] '>
                    ...
                    <div className='md:w-[17.4rem] mr-[16px]'>Total availble</div>
                ...
                {loading ? <Skeleton className='hidden md:block' loading={loading} active></Skeleton> :
                    <div className='hidden md:block w-[100%] mt-[64px] rounded-[1px] border border-solid border-[#b0b6bd] font-bold ' >
                        <div className='mt-[2rem] ml-[2rem] text-[2.4rem] ml-[1rem]'>Scroll assets</div>
                ...
                {loading ? <Skeleton className='md:hidden' loading={loading} active></Skeleton> :
                    <div className='md:hidden  px-[32px] py-[32px]  mt-[64px] rounded-[1px] border border-solid border-[#b0b6bd] font-bold ' >
                        <div className='text-[16px] font-bold mb-[16px]'>Scroll assets</div>
                        {data && data.map((item, index) => (<div>
                            ...
                            <div className='mt-[21px] text-right mb-[16px]'>
                                <div className='inline-block  px-[10px] py-[5.5px] text-[bold] text-[14px] rounded-[1px] border border-solid border-[#b0b6bd] cursor-pointer' onClick={() => router.push('/Details?asset=' + item.asset[0] + '&&net=Scroll')}>Details</div>
                            </div>
